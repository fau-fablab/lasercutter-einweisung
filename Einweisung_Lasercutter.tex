...
\begin{document}
	...
	\begin{itemize}
		...
		\item Ebenfalls nicht erlaubt sind leichtentzündliche oder gefährliche Dinge, wie z.B. gefüllte oder leere Feuer\-zeuge (Behältnisse von brennbaren Stoffen). Wenn das Gehäuse nicht aus Metall ist, muss bei Handys der Akku herausgenommen werden.
		\item Lasercutter nur unter ständiger Beobachtung betreiben! Solange der Laser aktiv ist, muss immer eine Person \emph{direkt} beim Lasercutter sein und in den Bauraum sehen. Für vollständig unbrennbare Materialien (Glas, nicht lackierte Metallteile) gibt es eine Ausnahme, wenn vorher alle brennbaren Reste aus dem Schneidetisch entleert wurden.
		\item Alle Arbeiten erfolgen auf Eigenverantwortung und eigenes Risiko. Das FabLab haftet nicht für deine Beschädigungen, auch nicht wenn dein schickes Handy beim Gravieren kaputt geht.
		\item Die Schrauben zur Befestigung des Schneidetischs (Wabengitter) dürfen im Notfall gedreht werden, ansonsten nichts schrauben oder verstellen. \todo{Diese durch Rändelschrauben ersetzen, um Verwechslungen zu vermeiden, und grün markieren (z.B. Plotterfolie um die Soll-Position) damit wir den gleichen Text wie beim Zing verwenden können}
		\item \nurZing Die zwei grün markierten Rändelschrauben am Schneidetisch dürfen nach entnahme aus dem Gerät von Hand gedreht werden, ansonsten nichts verstellen. % Betätigen der grünen Schrauben ist nötig, um den Schneidetisch zu reinigen! Deshalb kann man es nicht ganz verbieten.
		...
		\item Magnete sind im Bauraum und im Zweifel auch in der nähe des Lasers nicht gestattet. 
		\item \nurLTT \todo{...}
		\item \nurLTT Sofortiges Abschalten des Lasercutters geht nur mit dem Not-Aus-Schalter. \laserLTTPause fährt die nächsten Linien noch zu Ende. Dies braucht aber seine Zeit (Siehe 1.2 "TODO:")
	...
	\begin{itemize}
		...
		\item Bei weiterer Eskalation die Feuerwehr rufen, anwesende Personen warnen und mit allen den Raum verlassen (Erstickungsgefahr durch CO\subscript{2} und durch Rauch) Gegebenfalls anweisungen der Feuerwehr beachten.
	\end{itemize}
	...
	\subsection{\nurLTT Eigenheiten und Marotten} %Vielleicht möchte man das für beide Laser formulieren?
	Unser neuer Laser hat einige Rotienen, die bei Unwissenheit darüber zur Zerstörung des Gerätes führen können. Deshalb hier eine kleine Auflistung der zu beachtenen Punkte:
	\begin{itemize}
		\item Bevor das Geräte eingeschalten wird und während des Benutzens, muss sichergestellt sein, dass sich der Laserkopf in seiner Ebene komplett frei bewegen kann, ohne auf Hindernisse (zum Beispiel eingelegtes Lasermaterial) zu stoßen!
		\item Bei der automatischen Fokuseinstellung ist darauf zu achten den höchsten Punkt der zu lasernen Fläche für den Fokus zu nutzen. Im Zeifelsfall jedoch die höchste Stelle des kompletten Werkstücks. (Der Laser erlaubt nur einen Fehler unter 4mm)
	\end{itemize}
	...
	\begin{itemize}
		...
		\itemCheck Holz, Pappe, Papier und ähnliche auf Holz basierende Werstoffe ohne giftige Zusätze (HDF, Hartholz, Sperrholz...)
		\itemCheck "trockene" Nahrungsmittel, soweit bekannt wie zum Beispiel Äpfel (nur gravieren), Butterkeks ohne Schokolade, Brezen...
	...
	\subsection{Material}
	Material gibt es im Lagerregal und in den Schubladen nebenan in der Elektrowerkstatt. Spezielle Verarbeitungshinweise finden sich im Wiki. Platten kann man einfach auf den Schneidetisch legen, entstehender Rauch wird dann nach unten und hinten abgesaugt. \nurLTT Nach dem Job einige Minuten warten, da unsere momentane Absauglösung nicht schnell genug ist um Abgse zuverlässig abzusaugen. An den Stellen, an denen der Laser auf das Gitter trifft, gibt es beim Schneiden Reflexionen, die z.B. bei durchsichtigem Acryl störende kleine Macken an Schnittkanten erzeugen können. Um dies zu umgehen, gibt es allerlei Tricks mit Abstandshaltern, die im Wiki dokumentiert sind.
	...
	\subsubsection{\nurLTT}
	\textbf{WICHTIG! Achte zu jeder Zeit darauf, dass sich der Laserkopf in seiner XY-Ebene frei bewegen kann, ohne dabei auf irgendwas zu stoßen! Bewege deshalb den Tisch vorher weit genug nach unten.} \\
	Um den Fokus an diesem Gerät einzustellen, hat der Hersteller eine hilfreiche Funktion eingebaut. Um diese zu nutzen, muss der Laserkopf vorher manuell mittels den Pfeiltasten die um das Haussymbol angeordnet sind auf die höchste Stelle der zu bearbeitenen Fläche bewegen. Bei gekrümmten Flächen sollte sichergestellt sein, das der Fokusabtaststift der rechts von der Laserlinse verbaut ist, über dem höchsten Punkt steht. Beachte dabei, dass der Laser nach dem Fokusieren über diesen vermeindlich höchsten Punkt maximal 4mm Freiraum hat, bevor es zu einem Crash kommt. Steht der Fokusabtaststift über der gewünschten Stelle, kann der Prozess gestartet werden. Dafür wird der Knopf mit dem Lupensymbol zwischen den Tasten für die Tischverstellung betätigt. \textbf{Für den gesammten Verlauf des Prozesses muss eine Hand über dem Not-Aus Schater breit stehen um das Gerät vor einem Crash zu stoppen!}  Nach erfolgreicher Fokuseinstellung fährt der Lasertisch auf den richtigen Fokusabstand und bleibt danach in dieser Position. Da nach ist unbedingt zu Prüfen, ob der Laserkopf beim Verfahren mit dem Lasermaterial kollidieren kann. Gegebenenfalls das Fokusieren wiederholen. \\
	\textbf{Sollte der Laser gestoppt werden, nachdem dieser in eine Position gefahren ist, bei der Laserkopf nicht in XY-Richtung bewegt werden kann, muss vor erneutem Einschalten der Tisch per Hand heruntergefahren werden. Auf keinen Fall den Laser vorher einschalten!}
	...
	\begin{itemize}
		\item \textbf{nach jedem Job:} Werkstück und Reste aus dem Bauraum entfernen. Werkzeuge, Messmittel und Hilfsmittel wieder an ihren zugedachten Platz bringen.\\
		\nurZing Wenn mehr als eine Handvoll Reste im Schneidetisch liegen, diesen ausleeren: Schneidetisch einfach herausnehmen (dazu müssen keine Schrauben gelöst werden). Vorderklappe des Schneidetischs entfernen (grüne Rändelschrauben lösen) und den Schneidetisch entleeren. Dann den Schneidetisch wieder zusammenbauen und so in den Lasercutter einsetzen, dass er fest sitzt.
		\item \textbf{letzter Job} Vor dem Ausschalten des Lasers sichergehen, das der Bauraum frei von Restmaterialien oder Abgasen ist. Gegebenfalls die Lüftung etwas nachlaufen lassen. Absaugung ausschalten (siehe \ref{sec:absaugung}).
	\end{itemize}
