...
\begin{document}
	...
	\nurLTT Es ist zu beachten, dass die Kamera noch ziemlich ungenau ist. Diese beziehen sich zur Zeit noch auf einige Millimeter Unsicherheit! Bitte richte dich am Besten an den Linealen am Rand des Wabengitters und z.B. VisiCut.
	...
	\begin{tabularx}{\textwidth}{|X|X|X|}
		...
		& VisiCam ist verkalibriert &	Wenn du nicht an Rechner ws01 sitzt, lade aktuellere Einstellungen herunter (Optionen - empfohlene Einstellungen - Erlangen). Wenn das nichts bringt, muss auf ws01 Kalibrierungsprogramm in VisiCut durchgeführt und dann die Einstellungen veröffentlicht werden (Betreuer/Admins fragen).  \\ \hline
	...
	\subsection{Riemenspannung / zittriges Schnittbild}
	Bei zittrigem Schnittbild ist typischerweise der Riemen zu locker. Siehe Wartungshandbuch des Herstellers. Leider ist dieses ziemlich wirr.
	
	Wenn man lange genug im Wartungshandbuch sucht, findet man folgende Angabe: Der X-Riemen ist richtig gespannt, wenn er sich \enquote{in der Achsmitte ca. 1-1,5 cm von der Achse wegziehen lässt}. Nicht Übermäßig fest spannen. Y-Riemen muss 10 \% mehr Spannung haben als der X-Riemen (was auch immer das heißen soll). Zum Prüfen immer in die Mitte der Arbeitsfläche fahren (P2). 
	
	\subsection{Optik putzen}
	...
	Spiegel reinigen:
- Spiegel 5 (Spiegel im Linsenwagen)
	- Anleitung dazu ist in der Freigabe unter: Dateifreigabe > 1_fablab > 1_Geraete > Laser > Anleitung Novograv (LTT) iLaser 4000 : Wartungshandbuch_iLaser_05:2018
	- Wartungshandbuch hier falsch: der Spiegel kann nicht herausgezogen werden, deshalb statdessen den Spiegel vorsichtig mit Wattestäbchen und Linsenreiniger ähnlich der Linse durch die seitliche Öffnung reinigen.
- Spiegel 4 (Am Ende der Schiene, auf der der Linsenwagen fährt)
	- Nach gleicher Methode mit Wattestäbchen und Linsenreiniger
- Spiegel 3 und Fenster nach unten
	- Nach gleicher Methode mit Wattestäbchen und Linsenreiniger

- Lüfter der Kühlung säubern
	- Stecker ziehen!
	- Seitenklappe unten mit Schlüssel aus der Betreuerschublade öffnen
	- Lüfterfilter reinigen, wenn sich diese zusetzten verdreckt der Laser auch unten beschleunigt, da er sich dreckige Luft zum kühlen einzieht.

Nach der Wartung kontrollieren, dass KEINE GEGENSTÄNDE im Laser liegen, BEVOR man ihn anschaltet, sonst kollisionsgefahr!

Nur wenn erforderlich:

- Spiegel 1 & 2 kontrollieren und säubern:
	- Stecker ziehen!
	- Seitenklappe unten mit Schlüssel aus der Betreuerschublade öffnen
	- Nach Wartungshandbuch 4.6 (S. 16) vorgehen: Abdeckung laut Wartungshandbuch öffnen und kontrollieren ob Spiegel & Einkoppelspiegel sauber sind, NICHT verstellen.


	\subsection{Vorgehen nach (vermutlicher) Kollision}
	Obacht, diese Aufzählung ist grob vereinfacht und sollte noch verbessert und detailliert werden. Bitte unbedingt auch die jeweiligen Abschnitte des Wartungshandbuchs lesen!

	\begin{itemize}
		\item Netzstecker (Kaltgerätekabel) ziehen!
		\item Z-Achse prüfen: Sollte der Laser ausgeschaltet werden (auch Not-Aus) und dann in einer Position stehen, bei der Laserkopf nicht in der (XY-)Ebene frei bewegt werden kann, den Tisch vorher per Hand herunterfahren: Netzstecker ziehen (Kaltgerätekabel rechts an Laser), Wartungsklappe vorne unten mit Schlüssel öffnen, durch Schieben des Zahnriemens den Tisch hoch/runter bewegen. Ein Hinweiszettel am Boden des Gehäuses zeigt die nötige Bewegungsrichtung für das Herunterfahren des Tisches.
		\item Fokusstift prüfen: Muss frei herunterfallen, wenn man ihn hochhebt, und darf nicht krumm o.\,ä. sein.
		...
		Wenn das nicht in Ordnung, ist die Achse ausgehängt und muss erst wieder eingehängt werden. Wenn dies nicht durch leichtes Wackeln von selber passiert, muss sie dazu teilweise demontiert werden. Siehe Wartungshandbuch unter \enquote{9. X-Achse ausgehängt}.
		\item Die Rollen dürfen nicht frei in der Luft drehen, sondern sollen auf der Nut aufliegen. Das prüft man durch vorsichtiges Drehen mit der Hand. Richtig ist es, wenn sie sich nicht frei drehen lassen ohne dass sich der Wagen bewegt, bzw. man erst etwas kräftiger drehen muss bis sie merklich durchrutschen.
		
			Wenn sich eine der Y-Rollen frei dreht, dann ziemlich sicher die Ausrichtung der Achsen falsch. Bei der X-Achse kann sich die Rolle eigentlich nur dann frei drehen, wenn der Wagen nicht richtig eingehängt ist.
		\item An diesem Punkt sollten die Achsen zumindest näherungsweise rechtwinklig zueinander sein (grob mit Geodreieck/Augenmaß schätzen), zumindest nicht merklich zueinander verdreht.
		\item Die Lichtschranken der X- und Y-Achse prüfen, indem man von Hand die Achsen dort hin verschiebt und sicherstellt, dass das \enquote{Fähnchen} mittig in die Lichtschranke ragt. Beide Lichtschranken sitzen rechts hinten, die zweite etwas versteckt unterhalb der ersten. Siehe Wartungshandbuch unter \enquote{10. Sensoren der Parkposition prüfen}.
		\item Laser wieder einschalten, prüfen dass Referenzfahrt beim Einschalten funktioniert.
		\item Riemenspannung prüfen. (siehe vorher). Wenn diese nicht passt, sollte sie vor/während der Justierung der Rechtwinkligkeit korrigiert werden.)
		\item Rechtwinkligkeit der Achsen prüfen: \todo{Das ist zur Zeit leider ein Riesensch...-Aufwand, wir brauchen dringend eine bessere Lösung. Mal den Hersteller nach dem Aluwinkel fragen, den er bei der Inbetriebnahme dabeihatte, oder doch die Z-Begrenzung so setzen dass sie bis zur unteren Tischplatte geht.} Hier der aktuelle Stand.
			
		Als Referenz dient das Lineal des Lasertischs (\textbf{nicht} der Wabentisch, sondern die Platte darunter). Dieses wurde vom Hersteller exakt rechtwinklig eingebaut \todo{ist das tatsächlich hundertprozentig so? Nachprüfen} und darf deshalb auch nicht umgeschraubt werden. Der rote Laserpointer soll beim Verfahren in X-Richtung auf der Linealkante von ganz links bis ganz rechts genau auf der Kante bleiben \todo{wie genau? welche Toleranz? -- wir nehmen mal ca 0.5mm an} und nicht nach vorne oder hinten wegwandern.
		
		Man kann aber den Tisch nicht soweit hochfahren, weil wir die Z-Achse so begrenzen, dass bei eingebautem Wabentisch man mit dem Fokusstift nicht viel tiefer als der Wabentisch fahren kann. Also muss man zunächst die Begrenzung der Z-Achse temporär entfernen, weil wir direkt auf der unteren Tischplatte \enquote{lasern} wollen. Dazu mal wieder Netzstecker ziehen, Wartungsklappe unten aufmachen, Fähnchen der Z-Achse mit 3mm Inbus aufschrauben und wegschieben, Wartungsklappe wieder zu. Strom an.
		
		Jetzt Lineal prüfen wie oben beschrieben. Wenn es nicht passt, siehe Wartungshandbuch \enquote{5.2 Rechtwinkligkeit der X- und Y-Achsen}.
		
		Nun die Z-Begrenzung wieder auf vorherigen Bereich: Tisch auf ungefähr die Höhe der vorherigen Z-Begrenzung fahren, wieder Netzstecker ziehen, Z-Begrenzung wieder einbauen: blauer Strich = Oberkante Fähnchen, sicherstellen dass das Fähnchen mittig in die Lichtschranke passt.
		
		Wartungsklappe zu und Strom an.
		
		\item Jetzt passt wieder alles.
	\end{itemize}
