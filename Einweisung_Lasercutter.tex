...
%  \item Absaugung ausschalten, wenn sie das Feuer weiter anfacht.
% \item Wenn es immer noch brennt: Soweit möglich, Material über Vorderklappe entnehmen (oben ist der Arm, an dem die Linse fährt, im Weg).
 \item Letzer Ausweg ist das Löschen mit dem CO\subscript{2}-Löscher in kurzen Sprühstößen.
...
\itemCheck erlaubte Kunststoffe: Acrylglas (PMMA), Overheadfolie (PET), Moosgummi (EVA-Schaum)
\itemCheck Holz, Pappe, Papier und Ähnliches (HDF, Butterkeks ohne Schokolade, ...)
\itemCheck schlecht laserbar aber trotzdem erlaubt: PE Polyethylen, PS Polystyrol
\itemCheck Polycarbonat (z.B. Bayer Vivak) bis 1mm Dicke
\end{itemize}
...
\itemCross PC Polycarbonat dicker als 1\,mm, weil es beim Lasern spritzt
...
% TODO verifizieren, ob bei Seitengröße != Treibereinstellung, oder Treibereinstellung=Seitengröße!=609x304 Probleme.

%TODO TilmanAushang behauptet: Eigene Dokumente (in A4 o.Ä.) landen irgendwo in der Mitte des Tisches. -> Ausprobieren, wo die landen
% das kommt davon, dass ICH (Phi) das gesagt habe. Das hab ich so ausprobiert, beim corel ist das so, wenn man die Geräteresetfunktion verwendet und NICHT einen eigenen Nullpunkt festlegt. Dieser wird trotz verstellter ``Papiergröße`` richtig verwendet.
% ??? (Max) verstehe ich nicht. Werde ich ausprobieren.
...
Es ist verfügbar unter \url{https://hci.rwth-aachen.de/visicut}. Die aktuell verwendeten Konfigurationen für VisiCut sind unter \url{https://github.com/fau-fablab/visicut-settings} dokumentiert. % TODO wie updated man das



\subsubsection{VisiCam}
%TODO verbessern, ...

VisiCam ist ein in VisiCut eingebettetes Programm, welches es ermöglicht, seine Vektorzeichnung auf einem Bild des eingelegten Materials zu platzieren. Dazu wird von einer Kammera, die an der Klappe des Lasercutters angebracht ist, ein Bild geschossen, das anhand der 4 Passermarken an den 4 Ecken der Arbeitsfläche des Lasers ausgerichtet und beschnitten wird. Das Bild kann durch Klicken des Buttons \button{Capture Photo} aktualisiert werden. Nun können die Teile, die gelasert werden sollen, je nach Kalibrierung bis auf etwa 2mm genau am Bild ausgerichtet werden, was vor Allem bei Reststücken nützlich ist. Vor dem Lasern ist noch zu beachten, dass der Laserkopf in den Nullpunkt des Lasercutters zurückgefahren ist. Dazu drückt man am Laser die Taste \laserXyAus und danach \laserReset
...
%TODO optional: Bild mit Kegel, 2mm in beide Richtungen
...
Anstatt den Nullpunkt selber zu wählen, kann man auch den Maschinennullpunkt (linke obere Ecke des Tisches) verwenden. Dazu drückt man \laserReset anstatt Start, der Lasercutter sucht dann den Nullpunkt und fährt anschließend den Arm aus dem Weg.

%TODO Nullpkt darf keinesfalls links vom Maschinennullpunkt liegen! Gravieren rummst an die Wand!
