...
\begin{document}
	...
	\nurLTT Die Kamera ist eher ungenau, 5-10mm Unsicherheit sind möglich! Ein noch genauerer Weg wird im nächsten Abschnitt erklärt.

	\nurZing Vor dem Lasern muss der Laserkopf in den Nullpunkt des Lasercutters (links oben) zurückgefahren werden. Dazu drückt man am Laser die Taste \laserZingXyAus und danach \laserZingReset. Nach dem Start ist der Laserkopf automatisch im Maschinennullpunkt.

	Fehlerbehebung siehe \cref{sec:wartung-visicam}.

	\subsubsection{Genauere Position -- ohne Kamera}
	\nurLTT Wenn du ganz genau positionieren willst, dann kannst du den Lasercutter mit den Pfeiltasten genau auf einen Punkt bewegen und dann die Position im Menü ablesen: Schalte den roten Hilfslaser an um den Punkt zu sehen (Knopf mit Lasersymbol unterhalb von \laserKnopf{P2}). Bewege den Laser mit den Pfeiltasten an den gewünschten Punkt. Gehe mit \laserLTTExit ins Hauptmenü, dann mit mehrmals \laserLTTNextJob den Punkt \laserDisplay{4. System Setup} auswählen und \laserLTTOkay. Drücke wieder mehrmals \laserLTTNextJob bis \laserDisplay{Position Status} ausgewählt ist und dann \laserLTTOkay. Jetztwird die exakte Position des Laserpunkts angezeigt (X: Abstand vom linken Rand, Y: Abstand von oben). Diesen Wert kannst du in Inkscape oder VisiCut zur Positionierung verwenden. Zurück ins Hauptmenü mit \laserLTTExit \laserLTTExit.
...
\end{itemize}
	...
	Wegen technischer Randbedingungen der Windows-IT wird das Nutzerprofil von fufablab05 regelmäßig automatisch gelöscht. Daher wird das git-repo über ein Skript manuell vom Nutzerprofil auf ein Netzlaufwerk gesichert bzw. wiederhergestellt.

	\paragraph{Vorbereitung: Git-Repo wiederherstellen:} Am oben beschriebenen Rechner und Benutzer Visicut schließen, C:/Users/fufablab05/.visicut/ öffnen. Wenn dort \textbf{kein} Ordner .git existiert, dann Fenster schließen, W:/visicut-backup-AUTOMATIC/.visicut/ öffnen und dort Rechtsklick auf fablab-fufablab05-backup-restore.ps1, Mit PowerShell ausführen, ggf. Sicherheitsabfrage mit J und Enter bestätigen.
	...
	\paragraph{Nachbereitung: Git-Repo sichern:} Visicut schließen. Alle Explorer-Fenster schließen. C:/Users/fufablab05/.visicut/ öffnen und dort Rechtsklick auf fablab-fufablab05-backup-restore.ps1, Mit PowerShell ausführen, ggf. Sicherheitsabfrage mit J und Enter bestätigen. (Das Skript erkennt automatisch ob es Backup oder Restore machen soll: Wenn der .git-Ordner existiert dann macht es eine Sicherung, sonst macht es eine Wiederherstellung. Um Fehler mit gesperrten Dateien zu vermeiden ist es wichtig das Skript zwecks Backup von C:/... zu starten und zwecks Restore von W:/..., obwohl es das gleiche Skript ist.)
	...
	Im Deckel des Lasercutters ist ein Raspberry Pi mit Kameramodul montiert. Getrennt davon laufen auf dem FabLab-Server die Programme Undistort-Webcam und VisiCam.
	Undistort-Webcam holt sich das Kamerabild und rechnet es von \enquote{Fischaugen-Linse} auf \enquote{ideale Linse} um, sodass gerade Linien wieder gerade sind. Als nächstes holt sich VisiCam das Ergebnis, erkennt die kreisförmigen Markierungen im Laser, und korrigiert damit die Bewegungen der Kamera. (Der Deckel des Lasercutters bleibt nie in der exakt gleichen Position stehen. Das fertige Bild von VisiCam sieht am Ende trotzdem immer gleich aus.)
	VisiCut holt dieses Bild von VisiCam und rechnet es dann von \enquote{schiefen Pixeln} in rechteckige Millimeter um.

	\paragraph{Prüfung von VisiCam} Gehe als erstes die folgende Checkliste \textbf{ganz} durch:
	\begin{enumerate}
		\item Der Laser ist an.
		\item Bei mehrmaligem Öffnen und Schließen öffnet sich der Laser-Deckel über die eingebaute Gasfeder vollständig und immer ähnlich weit. (Wenn die Gasfeder schwächelt, dann öffnet der Deckel manchmal nicht weit genug und der Marker ist außerhalb des eingestellten Bildbereichs.)
		...
		\item Der Kamera-Server (Raspi im Lasercutter) liefert ein Bild: \url{http://lasercam.lab.fablab.uni-erlangen.de:8080/}
		\item Die Linsen-Entzerrung (Undistort-Webcam) empfängt dieses Bild: Raw image unter \url{https://fablab.fau.de/visicam-undistort} anklicken, es erscheint das Bild.
		\item Die Linsen-Entzerrung verarbeitet es: Processed image unter gleicher Adresse, es erscheint ein ähnlich aussehendes Bild mit leicht krummem Rand.
		\item Hinweis: Kalibrierung der Linsen-Entzerrung ist \textbf{nicht} notwendig außer wenn die Kamera-Linse getauscht wurde. Wenn doch, dann ist es direkt auf obiger Webseite erklärt --- alte Kalibrierdaten löschen, Schachbrettmuster ausdrucken, Kalibrierfotos mit dem Schachbrettmuster an verschiedensten Stellen des Kamerabilds machen, Kalibrierung ausrechnen lassen, fertig. Die Kalibrierung ist dann erfolgreich wenn gerade Linien (z.B. Lineal) auf dem verarbeiteten Bild auch wieder gerade Linien sind (mit höchstens ca. 1 mm Toleranz).
		\item VisiCam zeigt das entzerrte Bild: Linke Spalte (raw image) von \url{https://fablab.fau.de/visicam}
